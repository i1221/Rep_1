...
namespace SimpleHttpServer
{
    public class HttpProcessor
    {
        ...
        #region Events
        // public event Action<string> RequestContent;
        public event Action<HttpRequest> RequestContent;
        // OnNewData?.Invoke(prgrBar1Val);
        ...
        // это форматирует ответ HTTP ... (this formats the HTTP response...)
        private static void WriteResponse(Stream stream, HttpResponse response) {            
            if (response.Content == null) {           
                response.Content = new byte[]{};
            }

            // по умолчанию текст/html тип контента (default to text/html content type)
        ...
        {
            // Читать строку запроса (Read Request Line)
            string request = Readline(inputStream);

            string[] tokens = request.Split(' ');
            if (tokens.Length != 3)
            {
                throw new Exception("неверная строка HTTP-запроса (invalid http request line)");
            }
            string method = tokens[0].ToUpper();
            string url = tokens[1];
            string protocolVersion = tokens[2];

            // Читать заголовки (Read Headers)
            ...
            while ((line = Readline(inputStream)) != null)
            {
                ...
                if (separator == -1)
                {
                    throw new Exception("неверная строка заголовка http: (invalid http header line:) " + line);
                }
